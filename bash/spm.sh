...
{
    if [[ $COMP_CWORD == $1 ]]; then
        COMPREPLY=( $(compgen -W "reset clean tools-version generate-xcodeproj init update unedit edit config completion-tool describe experimental-api-diff _format dump-package show-dependencies fetch resolve -Xcc -Xswiftc -Xlinker -Xcxx --configuration -c --build-path --chdir -C --package-path --multiroot-data-file --sanitize --disable-prefetching --skip-update --disable-sandbox --disable-package-manifest-caching --version --destination --verbose -v --no-static-swift-stdlib --static-swift-stdlib --force-resolved-versions --disable-automatic-resolution --enable-index-store --disable-index-store --enable-pubgrub-resolver --use-legacy-resolver --enable-parseable-module-interfaces --trace-resolver --jobs -j --enable-test-discovery --enable-build-manifest-caching --emit-swift-module-separately" -- $cur) )
        return
    fi
    case $prev in
        (-Xcc)
